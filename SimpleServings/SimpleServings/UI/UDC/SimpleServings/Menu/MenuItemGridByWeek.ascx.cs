...
{
    public partial class MenuItemGridByWeek : System.Web.UI.UserControl
    {
           ...
           // Session["pageName"] = pageName;
               ...
               foreach (Control c in Page.Controls)
                {
                    ...
                    {
                        if (c2.GetType().ToString() == "System.Web.UI.WebControls.LinkButton")
                        {
                            if (c2.ID.Contains("lnkbRemove"))
                            {
                                ((LinkButton)c2).Visible = false;
                            }
                        }
                    }
                }
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (Request["MenuID"] != null)
                {
                    ...
                    string userControlName = this.ID.ToString();  //get the userControl's  ID


                    switch (userControlName)
                    {
                        case "MenuItemGridByWeek2":
                            PopDropDowns("2");

                            PopGrid(menuID);
                            break;
                        case "MenuItemGridByWeek3":
                            PopDropDowns("3");

                            PopGrid(menuID);
                            break;
                        case "MenuItemGridByWeek4":
                            PopDropDowns("4");

                            PopGrid(menuID);
                            break;
                        case "MenuItemGridByWeek5":
                            PopDropDowns("5");
                ...
                if (row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    for (int i = 0; i < menuDays.Count; i++)
                    {
                        ...
                        try
                        {
                           ...
                           // System.Web.UI.WebControls.LinkButton linkBtnRemove = row.FindControl("lnkbRemove") as LinkButton;

                           //linkBtnRemove.CssClass = ""
        ...
        // following is for design view <%# CanDelete %>, <%# CanAlternate %>
        ...
        protected void gvMenuItems_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            //Session["pageName"] = pageName;
                ...
                //{
                    ...
                    //LinkButton btnRemove = e.Row.FindControl("lnkbRemove") as LinkButton;

                    //btnRemove.Visible = false;
                    //LinkButton btnAlte = e.Row.FindControl("lnkBAlternate") as LinkButton;
                    //btnAlte.Visible = false;
               // }
